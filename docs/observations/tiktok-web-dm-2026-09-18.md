...
| **Realtime** | `wss://im-ws.tiktok.com/ws/v2` | pbbp2 frames (below) | **[Obs]** sync frames only |
...
- **Inbound [Obs]**: field 5 header carries `X-Method: PayloadRelatedMethod`,
  ...
  field 6 = `"gzip"`, field 8 gunzips to a per-topic sync list (`(topic, idx,
  read-state, message-id-cursor)`). **No DM message body** — the inbox was empty.
...
| `messages_*` with pagination | **gap** | inbox empty; parser tested against a synthetic `messages_synth` fixture shaped from the mobile IM envelope |
| `ws_inbound_dm` decoded to text | **gap** | only sync frames captured (empty inbox); `ws_frontier_sync.json` holds a real sync frame; decode target pinned in `frontier-fields.md` |
...
**G2 result:** 2 of the 5 required kinds captured live with real data (contacts,
profile_other); `conv_list` present but empty; `messages_*` and `ws_inbound_dm`
are gaps caused by the empty inbox, not by the tool. The parsers are still built
and tested — against the real fixtures where they exist and against synthetic
fixtures (shaped from these real envelopes) for the message body. Re-run the
capture once a DM exists to fill the two gaps. Proceeding to Task B.
